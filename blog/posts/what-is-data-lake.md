...
This raw → cleaned → curated progression is the same idea that the lakehouse world formalized as the **medallion architecture** (Bronze, Silver, Gold). The zones exist to answer a question paths alone cannot: for any given dataset, *how processed and trustworthy is this?* An ML engineer building features wants the cleaned zone; an analyst publishing a KPI wants the curated zone; a data engineer debugging an ingest issue goes back to raw.
