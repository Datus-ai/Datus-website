...
The operating model behind that store is [contextual data engineering](/blog/contextual-data-engineering/). For the modeling object the agent retrieves most often, see [what a semantic model is](/blog/what-is-semantic-model/). For the category this memory serves, see [what a data engineering agent is](/blog/what-is-data-engineering-agent/). When a named KPI should hit a worker that cannot invent SQL, see [Datus subagents](/blog/introducing-datus-subagents/). When generation should emit portable [OSI (now Apache Ossie)](/blog/open-semantic-interchange-osi/) instead of backend YAML, see the [OSI semantic adapter](/blog/datus-osi-semantic-adapter/). Read the 0.3 docs when you are ready to wire storage; use this page when you need to explain to a teammate what actually sits under the chat.
