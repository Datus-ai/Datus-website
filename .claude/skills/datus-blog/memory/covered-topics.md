...
- Images: 3 (blog/public/images/data-agent-sandbox/) — Datus Studio screenshots, pngquant-compressed. Originally Chinese-locale UI; the operator re-captured all three in English on 2026-09-11 (the correct fix — screenshots are evidence and must never be redrawn, translating-figures.md §1). mission-board-sandbox shows a different moment than the Chinese original, so its alt text and caption were rewritten to match. One Chinese SQL comment (`-- 门店分布`) remains inside that screenshot
...
- Target keyword: semantic layer based ontology (semantic layer ontology, Apache Ossie ontology, semantic layer for AI agents, ontology for AI data agents, concept paths, value domains)
