...
---
```
Optional `tags`: a comma-separated string (`tags: insight, release`) or a YAML
list — `build-blog.mjs` normalizes both to a lowercase, de-duplicated array, so
either form is fine (older posts use the list form).

The tag that currently does something is **`insight`**: it puts the post in the
**Product & Insight** rail at the top of `/blog`, reserved for deep pieces the
Datus team wrote themselves — first-person engineering accounts, field reports,
